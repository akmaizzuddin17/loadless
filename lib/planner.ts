export type Task = {id:string; title:string; course:string; due:string; minutes:number; priority:number; done:boolean};
...
export function validTask(t:unknown):t is Task {if(!t || typeof t!=='object')return false;const v=t as Task;return typeof v.id==='string'&&typeof v.title==='string'&&v.title.trim().length>0&&v.title.length<=120&&typeof v.course==='string'&&v.course.length<=60&&typeof v.due==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(v.due)&&!Number.isNaN(Date.parse(v.due+'T12:00:00'))&&Number.isInteger(v.minutes)&&v.minutes>=5&&v.minutes<=1440&&[1,2,3].includes(v.priority)&&typeof v.done==='boolean';}
