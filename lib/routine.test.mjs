...
test('weekly overnight routine protects previous-day spill only',()=>{const sleep=[{id:'s',name:'Sleep',start:'23:00',end:'07:00',days:[1]}];assert.deepEqual(freeWindows(sleep,0,'2026-09-14'),[{start:0,end:1380}]);assert.deepEqual(freeWindows(sleep,0,'2026-09-15'),[{start:420,end:1440}]);assert.deepEqual(freeWindows(sleep,0,'2026-09-16'),[{start:0,end:1440}]);});
test('weekday commitments change future suggestions and invalid weekdays are rejected',()=>{const week=[{id:'a',name:'Monday',start:'00:00',end:'23:59',days:[1]},{id:'b',name:'Tuesday morning',start:'00:00',end:'10:00',days:[2]}];const starts=upcomingStarts([{...tasks[0],minutes:30}],week,undefined,'2026-09-14',0);assert.equal(starts.a.date,'2026-09-15');assert.equal(starts.a.start,600);assert.equal(validRoutine([{...week[0],days:[]}]),false);assert.equal(validRoutine([{...week[0],days:[7]}]),false);});
