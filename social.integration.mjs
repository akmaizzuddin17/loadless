...
await req('',a,{action:'friend',target:b,mode:'request'});await req('',b,{action:'friend',target:a,mode:'accept'});
const inviteSlot={id:'invite-slot-'+suffix,start:Date.now()+3600000,end:Date.now()+7200000};await other('together',b,inviteSlot);await other('together',a,{action:'invite',slot:inviteSlot.id,activity:'Movies'});await other('together',a,{action:'invite',slot:inviteSlot.id,activity:'Movies'},409);const incoming=(await other('together',b)).invites.find(i=>i.slot===inviteSlot.id);assert.equal(incoming.incoming,1);assert.equal(incoming.activity,'Movies');await other('together',a,{action:'respond',id:incoming.id,status:'accepted'},409);await other('together',b,{action:'respond',id:incoming.id,status:'accepted'});assert.equal((await other('together',a)).invites.find(i=>i.id===incoming.id).status,'accepted');await other('together',b,{action:'remove',id:inviteSlot.id});assert.equal((await other('together',a)).invites.find(i=>i.id===incoming.id).status,'cancelled');await other('together',a,{action:'invite',slot:inviteSlot.id,activity:'Movies'},403);console.log('PASS: friend invite delivery, duplicate prevention, recipient-only accept, sender status and cancellation on availability removal.');
